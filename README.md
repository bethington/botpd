...
- PD2 needs to be in English Language,
- Botpd currently works with 720p PD2 window (will be adjusted automatically on auto settings)

### 1) Graphics and Gameplay Settings

All settings will automatically be set when you execute `main.exe` and press the hotkey for "Adjust PD2 settings" (default f9). It is not a 100% thing, in rare cases you might still have to fiddle around with your brightness. I suggest using the "Graphic Debugger" to verify your settings.
...
Open up PD2 and wait till you are at the hero selection screen. Make sure the char you running with is selected and will be in any of Act 3, 4 or 5 in the respective difficulty you set in the **params.ini** once the bot starts the game.

### 4) Start Botpd

You can either run from python. Follow [development.md](development.md) for that. Or you download the a prebuilt release [here](https://github.com/aeon0/botpd/releases) (the .rar file!). Start `main.exe` in the botpd folder. Focus your PD2 window and press the start key (default f11). You can always force stop botpd with f12. Note: Botpd will use the /nopickup command in the first game to avoid pickup up trash while traversing. This command will only allow item pickup when "show items" is active.
...
Botpd NIP (BNIP) is an extended version of Njaguar's Item Parser (NIP).
BNIP is compatible with NIP (with some minor exceptions as discussed below).

There is a default nip file that comes with botpd called "default.nip" inside config folder, you can add your own nip files by putting them inside config/nip with a file extension of .nip. Creating your own nip file also turns off the default.nip.
...
To check if you graphic settings are good and if the bot would pick up items there is a **Graphic Debugger Mode**. Start botpd and press F10 (Default key). This will open up a (mostly black) window. Start a game in PD2 and go to A5. You should see some templates with blue circles detected and scores printed out to the console. To check item finding, throw some items of different types on the ground. The debug window should show the item names with black background. If you throw an item on the ground that should be picked up, it will have a red circle. The console will print out the scores for each item that would be picked up. Scores should be well above 0.9 for these items.</br>
<img src="assets/docs/graphic_debugger.png" width="900">

## params.ini

All botpd configuration files are located in the __config__ folder. To ease the switch to new botpd versions, you can also overwrite any of the .ini fields in a **custom.ini** file. When a new version of botpd is released you just copy the file to the new version without having to port all your **params.ini** changes to the new version. Example:
...
| saved_games_folder       | [Optional] Defaults to `C:\Diablo2Project\ProjectD2`. Used to store configuration settings for `f9` / auto settings. |
| custom_loot_message_hook      | Add your message hook here (such as Discord channel) to get info about loot |
| custom_message_hook      | Add your message hook here (such as Discord channel) to get info about botpd status updates, discord webhook is default. |
...
| message_api_type         | Which api to use to send botpd messages.  Supports "generic_api" (basic discord), or "discord" (discord embeds with images). |
| break_length_m           | Break for `break_length_m` minutes every `max_runtime_before_break_m` minutes |
| max_runtime_before_break_m | ^ |
| pd2_path                 | [Optional] Path to `pd2.exe`. If not set, it will default to `C:\Diablo2Project\ProjectD2` when attempting to restart. |
| max_consecutive_fails    | Botpd will stop making games if the number of consecutive fails reaches this max value. |
| max_game_length_s        | Max game length in seconds. Botpd will attempt to stop whatever it's doing and try to restart a new game at specified interval. If this fails, botpd will attempt to shut down PD2 and Bnet. |
| restart_pd2_when_stuck   | Set to `1` and botpd will attempt to restart pd2 in the case that botpd is unable to recover its state (e.g: game crash). |
...
| [routes]     | Descriptions                                                             |
| ------------ | ------------------------------------------------------------------------ |
| order        | List of runs botpd should do. These will be run in the the order listed unless `randomize_runs` is set to 1. Possible runs: </br> run_trav, run_pindle, run_eldritch, run_eldritch_shenk, run_nihlathak (requires teleport), run_arcane (requires teleport), run_diablo (requires teleport, only hammardin)
